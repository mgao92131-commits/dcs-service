...
internal static class ParityVerifier
{
    ...
    private static void CompareHistory(string csvPath, string jsonPath, string tag)
    {
        ...
        Dictionary<string, object> root = Json(jsonPath); Dictionary<string, object> data = Dict(root, "data"); Dictionary<string, object> samples = Dict(data, "samples"); object[] newRows = Array(samples, tag);
        Equal(oldRows.Count, newRows.Length, "history sample count");
        for (int i = 0; i < oldRows.Count; i++)
        {
            Dictionary<string, object> row = (Dictionary<string, object>)newRows[i]; Equal(DateKey(oldRows[i][0]), DateKey(Text(row, "timestamp")), "history timestamp row " + i); Equal(oldRows[i][1], Invariant(row["value"]), "history value row " + i); Equal(oldRows[i][2], Text(row, "dataType"), "history dataType row " + i);
        }
        PrintEdges("History", oldRows.Count, oldRows.Count == 0 ? "(empty)" : oldRows[0][0], oldRows.Count == 0 ? "(empty)" : oldRows[oldRows.Count - 1][0]);
    }

    private static void CompareEvents(string oldPath, string newPath)
    {
        Dictionary<string, object> oldRoot = Json(oldPath); object[] oldRows = Array(oldRoot, "records"); Dictionary<string, object> newRoot = Json(newPath); object[] newRows = Array(Dict(newRoot, "data"), "events"); Equal(oldRows.Length, newRows.Length, "event row count");
        ...
        {
            Dictionary<string, object> a = (Dictionary<string, object>)oldRows[i]; Dictionary<string, object> b = (Dictionary<string, object>)newRows[i]; Equal(DateKey(Text(a, "dateTime")), DateKey(Text(b, "timestamp")), "event timestamp row " + i);
            for (int f = 0; f < fields.Length; f++) Equal(Value(a, fields[f]), Value(b, fields[f]), "event " + fields[f] + " row " + i);
        }
        string first = oldRows.Length == 0 ? "(empty)" : Cursor((Dictionary<string, object>)oldRows[0]); string last = oldRows.Length == 0 ? "(empty)" : Cursor((Dictionary<string, object>)oldRows[oldRows.Length - 1]); PrintEdges("Event", oldRows.Length, first, last);
    }
