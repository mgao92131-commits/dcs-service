...
"bin\ParityVerifier.exe" history tests\fixtures\history-old.csv tests\fixtures\history-new.json TAG
if errorlevel 1 exit /b 1
"bin\ParityVerifier.exe" event tests\fixtures\event-old.json tests\fixtures\event-new.json
if errorlevel 1 exit /b 1
exit /b 0
