...
try {
    ...
    $r = Invoke-Raw "GET /api/v1/info HTTP/1.1`r`nHost: localhost`r`n`r`n"; if ($r -notmatch '"historyMaxConcurrent":2' -or $r -notmatch '"eventMaxConcurrent":4' -or $r -notmatch '"historyStreamWindowMinutes":60' -or $r -notmatch '"eventStreamWindowMinutes":60') { throw "info failed: $r" }
    ...
    $r = Invoke-Raw "GET /api/v1/events?afterTime=2026-08-30T10%3A00%3A00&afterFracSec=1&afterOrd=1&sourceGeneration=G&to=2026-08-30T09%3A00%3A00 HTTP/1.1`r`nHost: localhost`r`n`r`n"; if ($r -notmatch "HTTP/1.1 400") { throw "event cursor with to before afterTime was accepted: $r" }
