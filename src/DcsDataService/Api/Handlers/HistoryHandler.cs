...
namespace DcsDataService.Api.Handlers
{
    public sealed class HistoryHandler : IApiHandler
    {
        ...
        public object Handle(HttpRequest request)
        {
            ...
            IDictionary<string, IList<DcsDataService.DeltaV.Historian.HistorySample>> samples = _c.Historian.ReadRaw(tags, start, end, max); int count = 0; Dictionary<string, object> wire = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase); foreach (KeyValuePair<string, IList<DcsDataService.DeltaV.Historian.HistorySample>> pair in samples) { List<object> rows = new List<object>(); for (int i = 0; i < pair.Value.Count; i++) { DcsDataService.DeltaV.Historian.HistorySample s = pair.Value[i]; rows.Add(new { tag = s.Tag, timestamp = s.Timestamp.ToString("yyyy-MM-ddTHH:mm:ss.fffffff", System.Globalization.CultureInfo.InvariantCulture), value = s.Value, dataType = s.DataType, deltaVStatus = s.DeltaVStatus, archiveStatus = s.ArchiveStatus, sequenceNo = s.SequenceNo, isHistoryHole = s.IsHistoryHole, isCRHole = s.IsCRHole, isManuallyDeleted = s.IsManuallyDeleted, isManuallyInserted = s.IsManuallyInserted }); } count += rows.Count; wire[pair.Key] = rows; } _c.Log.Info("History query tagCount=" + tags.Count + " sampleCount=" + count); return new { samples = wire, sourceTimeZone = _c.Config.SourceTimeZone };
        }
    }
}
